...
internal class FraudDetectConsumerServiceImpl(consumerFactory: KafkaConsumerFactory<Order>) :
    FraudDetectConsumerService,
    BaseConsumerServiceImpl<Order>(
        consumerFactory.build(
            "ECOMMERCE_NEW_ORDER",
            FraudDetectConsumerService::class.java,
            Order::class.java
        )
    )